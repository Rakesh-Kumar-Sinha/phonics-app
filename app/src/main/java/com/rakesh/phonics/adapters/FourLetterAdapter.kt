...
class FourLetterAdapter(
    private val context: Context,
    private val letters: ArrayList<FourLetter>
) : RecyclerView.Adapter<FourLetterAdapter.LetterViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LetterViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_four_letter, parent, false)
    ...
    override fun onBindViewHolder(holder: LetterViewHolder, position: Int) {
        ...
        holder.tvLetterThree.text = data.letterThree.lowercase() + "\n•"
        holder.tvLetterFour.text = data.letterFour.lowercase() + "\n•"
        holder.btnBothLetter.text =
            data.letterOne.lowercase() + data.letterTwo.lowercase() + data.letterThree.lowercase()+ data.letterFour.lowercase()
        ...
        holder.tvLetterThree.setOnClickListener {
            resetAllTextColors()
            lastClickedHolder = holder
            holder.tvLetterThree.setTextColor(Color.RED)
            val path = "alphabets/" + data.letterThree.uppercase() + ".mp3"
            playAudioFromAssets(path) {
                holder.tvLetterThree.setTextColor(Color.WHITE)
            }
        }
        holder.tvLetterFour.setOnClickListener {
            resetAllTextColors()
            lastClickedHolder = holder
            holder.tvLetterFour.setTextColor(Color.RED)
            val path = "alphabets/" + data.letterFour.uppercase() + ".mp3"
            playAudioFromAssets(path) {
                holder.tvLetterFour.setTextColor(Color.WHITE)
            }
        }
    ...
    private fun resetAllTextColors() {
        lastClickedHolder?.let {
            ...
            it.tvLetterThree.setTextColor(Color.WHITE)
        }
    ...
    class LetterViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val tvLetterThree: TextView = itemView.findViewById(R.id.tvLetterThree)
        val tvLetterFour: TextView = itemView.findViewById(R.id.tvLetterFour)
